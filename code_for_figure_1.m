...
%Display heat map + picture of the craniotomy (Figure 1D)
...
%Display examplar glomeruli (Figure 1E)
for G = glomToDisplay
    LL = zeros(7,37);
    for t = 1:nbTrials
...
%Plot the contra vs ipsi responses (Figure 1F)
...
%Plot the histogram for contra responses, fit a Gaussian curve (Figure 1F)
c = 0.005;
stp = [-0.02:c:0.25];
subplot(1,2,2)
hold on
...
%NOTE: Figure 1I is similar to Figure 1F, except we display the magnitude
%of neuronal responses instead of the actual responses.
...
%Plot nb of units responding to ipsi and/or contra stimulations (Figure 1H)
M = C{1};
...
figure
plot([Ionly Conly IandC noR], '-o')
xticks([1 2 3 4])
xticklabels({'I only','C only','I and C','No resp.'})

%Plot pie chart for percentage positive / negative responses (Figure 1H)
M = C{1};
M = M(:,1:15,:); %the 16th odor is the blank trial
R = A{1};
R = R(:,1:15,:);
...
%Plot the contra vs ipsi responses (Figures 1J)
load('tetrodeRecordings_OB_new')
M = C{1};
M = M(:,1:15,:); %the 16th odor is the blank trial
R = A{1};
